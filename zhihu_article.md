...
**先报一下底**：男 / 173cm / 67kg / 30 岁 / 自由力量训练 5 年 / 体脂 ~12-14% / 训练目标偏体型 + 力量平衡。常练场地不重要，普通商业健身房就够。这套东西能不能套到你身上，先看下面这张分层表：
...
| 点 | 说明 |
|---|---|
| 标题字符过滤 | 半角 `[GPT5.5]` 已实测可用；如果服务端报 `title contains unsafe characters`，再兜底改全角 `【】` |
| 批量限速 | 同一 (date, endpoint) 90 秒锁定；不同日期通常可短间隔，保守写入仍建议 sleep 95s |
